...
def get(*vargs):
    file_path, destination_path = vargs

    if not check_path_exists_in_hdfs(file_path):
        print(f"Error: {file_path} not found in the DFS.")
        return False
    ...
    for block_id in block_ids:
        datanode_id = get_datanode_id_from_block_id(block_id)
        if datanode_id is None:
            print(f"Error: Missing block {block_id} in the DFS.")
            return False
        block_paths[block_id] = Path(args.PATH_TO_DATANODES).joinpath(datanode_id).joinpath(block_id)
    ...
    for block_id, block_path in block_paths.items():
        with open(block_path, "r") as block_file:
            content = block_file.read().strip()
            output_file_path = output_path.joinpath(f"{block_id}.txt")
            with open(output_file_path, "w") as output_file:
                output_file.write(content)

    print(f"File '{file_path}' retrieved successfully and saved to '{output_path}'")
    return True
...
def mv(source_path, destination_path):
    ...
    source_components = list(filter(bool, source_components))
    source_name = source_components[-1]
    source_parent_path = '/'.join(source_components[:-1]) or '/'

    # Check if the destination parent directory exists
    '''if not check_path_exists_in_hdfs(destination_path):
        print(f"Error: Destination parent directory {destination_path} not found.")
        return False'''

    # Form the destination file path
    destination_file_path = f"{destination_path}/{source_name}"

    # Perform the move operation
    create_file_in_hdfs(destination_file_path)
    file_id = get_file_id_from_hdfs_file_path(source_path)
    args.FILE_INFO[file_id]['file_path'] = destination_file_path
    update_namenode_file_info()
    update_namenode_filesystem_info()
    log_namenode(f"MOVE {source_path} TO {destination_file_path}")
    print(f"File {source_path} moved to {destination_file_path}.")
